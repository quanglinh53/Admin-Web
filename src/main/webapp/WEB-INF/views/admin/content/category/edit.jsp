<%@ page isELIgnored="false" pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib uri="http://tiles.apache.org/tags-tiles" prefix="tiles" %>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<%@ page import="com.osp.common.Constant" %>
<section id="content">
    <section class="vbox">
        <section class="scrollable padder">
            <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                <li><a href="<%=request.getContextPath()%>/admin"><i class="fa fa-home"></i>&nbsp;Trang chủ</a></li>
                <li><a href="#">Quản lý nội dung</a></li>
                <li><a href="#">Chủ đề</a></li>
                <li><a href="javascript:void(0)">Sửa</a></li>
            </ul>
            <div class="m-b-md">
                <%--<h3 class="m-b-none">Chỉnh sửa chuyên mục</h3><br/>--%>
                <span style="color:red">${messageError}</span>
            </div>

            <section class="panel panel-default">
                <header class="panel-heading"><i class="fa fa-caret-square-o-right"></i> Chỉnh sửa chủ đề</header>
                <div class="panel-body" style="min-height: 600px;">
                    <form method="post" action="<%=request.getContextPath()%>/admin/content/category/edit" theme="simple"  enctype="multipart/form-data" class="form-horizontal" cssStyle="" validate="true">

                        <input name="id" maxlength="100" style="width:100%;" value="${item.id}"
                               placeholder="" class="form-control" type="hidden"/>
                        <div class="form-group">
                            <label class="col-sm-2 control-label" style="line-height: 30px">Nhóm chủ đề
                                <code>*</code></label>
                            <div class="col-sm-10">
                                <select id="groupTopic" name="parentId" class="select2-option" style="min-width:200px">
                                    <option value="">Lựa chọn</option>
                                    <c:forEach items="${groupTopics}" var="groupTopic">
                                        <option value="${groupTopic.id}" ${item.parentId==groupTopic.id?"selected":""}>${groupTopic.name}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        <div class="line line-dashed line-lg pull-in"></div>
                        
                        <div class="form-group">
                            <label class="col-sm-2 control-label" style="line-height: 30px">Tên chủ đề<span style="color: red">*</span></label>
                            <div class="col-sm-10">
                                <div class="input-group m-b">
                                        <span class="input-group-addon">
                                            <i class="fa fa-file-text" aria-hidden="true"></i>
                                        </span>
                                    <input name="name" maxlength="100" style="width:100%;" value="${item.name}"
                                           placeholder="" class="form-control"/>

                                </div>
                                <form:errors cssStyle="color: red" path="category.name" />
                            </div>
                        </div>
                        <div class="line line-dashed line-lg pull-in"></div>
                        <input type="hidden" id="status" name="status" value="0">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">Kích hoạt chủ đề</label>
                            <div class="col-sm-10">
                                <label class="switch">
                                    <input id="checkboxActive" type="checkbox" ${item.status==1?"checked":""} onclick="changeStatusActiveCategory()">
                                    <span></span>
                                </label>
                            </div>
                        </div>

                        <div class="line line-dashed line-lg pull-in"></div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label" style="line-height: 30px">Mô tả </label>
                            <div class="col-sm-10">
                                <div class="input-group m-b">
                                        <span class="input-group-addon">
                                            <i class="fa fa-rotate-right" style="font-size: 11px" aria-hidden="true"></i>
                                        </span>
                                    <input name="description" placeholder="" maxlength="500" value="${item.description}" style="width:100%;" class="form-control"/>
                                </div>
                            </div>
                        </div>

                        <div class="line line-dashed line-lg pull-in" style="clear:both ;margin-bottom: 50px"></div>
                        <div class="form-group">
                            <div class="col-sm-4 col-sm-offset-5">
                                <a href="<%=request.getContextPath()%>/admin/content/category/list" class="btn btn-default">Hủy bỏ</a>
                                <button type="submit" data-loading-text="Cập nhật" class="btn btn-primary">Cập nhật</button>
                            </div>
                        </div>
                    </form>

                </div>
            </section>
        </section>
        <footer class="footer bg-white b-t b-light"><small>GAME-FULL &copy; 2020</small></footer>
    </section>
    <a href="#" class="hide nav-off-screen-block" data-toggle="class:nav-off-screen" data-target="#nav"></a>
</section>

<script>
    function changeStatusActiveCategory() {
        if ($('#checkboxActive').is(":checked")) {
            $('#status').val(1);
        } else $('#status').val(0);
    }
</script>