...
public class GroupDaoImpl implements GroupDao {
    ...
    public Optional<PagingResult> page(String name, PagingResult page) {
        ...
        Long count=(Long)entityManager.createQuery("select count(gr.id) from com.osp.model.admin.Group gr where gr.groupName like :name").setParameter("name","%"+name+"%").getSingleResult();
        List<Group> list=entityManager.createQuery("select gr from com.osp.model.admin.Group gr where gr.groupName like :name",Group.class).setParameter("name","%"+name+"%")
    ...
    public Optional<List<Group>> loadAllGroup() {
        List<Group> items=entityManager.createQuery("Select gr from com.osp.model.admin.Group gr ",Group.class).getResultList();
        return Optional.ofNullable(items);
    }

    @Override
    public Optional<List<Group>> loadAllGroupOfUser(Long userId) {
        List<Group> items=entityManager.createQuery("SELECT gr FROM com.osp.model.admin.Group gr JOIN GroupUser gu ON gr.id=gu.groupId where gu.userId=:userId and gr.status=1",Group.class)
    ...
    public Optional<Boolean> deleteGroup(Integer id) {
        Query query=entityManager.createQuery("delete from com.osp.model.admin.Group gr where gr.id=:id").setParameter("id",id);
        query.executeUpdate();
        return Optional.ofNullable(true);
    }
    ...
    public Optional<List<String>> loadListAuthorityOfUserByUsername(String username) {
        List<String> items=entityManager.createQuery("SELECT au.authority FROM Authority au JOIN GroupAuthority ga ON au.id=ga.authority JOIN com.osp.model.admin.Group gr ON gr.id=ga.groupId JOIN GroupUser gu ON gr.id=gu.groupId JOIN User us ON us.id=gu.userId WHERE us.username=:username")
